...
class Pr extends CI_Controller {
    ...
    {   
        ...
        $this->M_permintaan->ubah_status_detail($_POST);
        //$this->session->set_flashdata('sukses','Permintaan detail pembelian sudah berhasil di hapus<br>Terimakasih');
        header('location:'.base_url().'permintaan/pr/detail/'.$id_pr); 
    }
    ...
    public function dataLampiran(){
        ...
        echo "<table class='table table-bordered table-striped' >
                <thead>
                  <tr>
                    <th width='30px'>No</th>";
        //echo "      <th width='70px'>Lampiran</th>";
        echo "      <th>Nama Lampiran</th>
            ...
            if(empty($lampiran)){
                ...
                foreach ($lampiran as $row) {
                    echo "<tr><td>$no</td>";
                   //echo "<td><img src='".base_url()."/assets/lampiran_pr/$row->file_lampiran' height='40px'></td>";
                    echo "<td>$row->keterangan_lampiran</td>";
                    echo "<td><button onclick='delete_lampiran($row->id_permintaan_lampiran)' class='btn btn-xs btn-danger' title='hapus lampiran'><i class='fa fa-trash'></i></button>";
                            
                    echo "<a class='btn btn-xs btn-info' href='".base_url()."assets/lampiran_pr/$row->file_lampiran' target='blank'><i class='fa fa-eye' title='lihat lampiran'></i> Lihat</a>
                          </td></tr>";
                    $no++;
                }
            }
    ...
    public function dataPo(){
        ...
        echo "<table class='table table-bordered table-striped' >
            ...
            $sql_po   = "SELECT * FROM abe_permintaan_po WHERE id_permintaan = '$no_permintaan'";
            //$sql_rapat      = "SELECT status, no_rapat FROM abe_rapat_new WHERE no_rapat = '$no_rapat'";
            $po = $this->db->query($sql_po)->result();
            //$rapat = $this->db->query($sql_rapat)->row_array();
            if(empty($po)){
                echo "<tr><td colspan='3'><center>Tidak ada PO / Memo</center></td></tr>";
            }else{
                $no = 1;
                foreach ($po as $row) {
                    echo "<tr><td>$no</td>";
                    echo "<td>$row->po_memo</td>
                          <td>";
                    echo "<button onclick='delete_po($row->id_pr_po)' class='btn btn-xs btn-danger' title='hapus po'><i class='fa fa-trash'></i></button></td></tr>";
                    $no++;
                }
            }
        echo "</table>";
    ...
    public function dataProgress(){
        ...
        echo "<table class='table table-bordered table-striped' >
            ...
            $sql_progress   = "SELECT * FROM abe_permintaan_progress WHERE id_permintaan = '$no_permintaan'";
            //$sql_rapat      = "SELECT status, no_rapat FROM abe_rapat_new WHERE no_rapat = '$no_rapat'";
            $progress = $this->db->query($sql_progress)->result();
            //$rapat = $this->db->query($sql_rapat)->row_array();
            if(empty($progress)){
                echo "<tr><td colspan='3'><center>Tidak ada Progress</center></td></tr>";
            }else{
                $no = 1;
                foreach ($progress as $row) {
                    ...
                    echo "<td>$row->progress</td>
                          <td>";
                    echo "<button onclick='delete_progress($row->id_pr_progress)' class='btn btn-xs btn-danger' title='hapus progress'><i class='fa fa-trash'></i></button></td></tr>";
                    $no++;
                }
            }
        echo "</table>";
    ...
    public function add_po()
    {
        ...
        $insert = $this->M_permintaan->save_po($data);
        echo json_encode(array("status" => TRUE));
    }
    ...
    public function add_progress()
    {
        ...
        $insert = $this->M_permintaan->save_progress($data);
        echo json_encode(array("status" => TRUE));
    }
    ...
    {        
        $this->M_permintaan->hapus_po($id);
        echo json_encode(array("status" => TRUE));
    }

    public function hapus_progress($id)
    {        
        $this->M_permintaan->hapus_progress($id);
        echo json_encode(array("status" => TRUE));
    }

    public function cetak_pr_kecil(){
        //$id = $this->uri->segment(4);
        $data['record'] = $this->M_permintaan->ambil_pr()->row_array();
        //$this->template->load('template','permintaan/form_detail_permintaan',$data);
        //$data['record'] = $this->M_rapat->ambil_notulen_rapat()->row_array();
        $this->load->view('permintaan/cetak_pr_kecil',$data);
    }

    public function cetak_pr_pending()
    {
        $this->load->view('permintaan/cetak_pr_pending');
    }
}
