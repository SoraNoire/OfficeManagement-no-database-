...
class M_permintaan extends CI_Model {
    ...
    {    
        ...
        $input = array('pr_diajukan'=>$diajukan,'id_permintaan'=>$id_permintaan);
        if(!empty($_FILES['file']['name']))
            {
    ...
    public function proses_mengetahui()
    {
        ...
        $data = array('status_pr'=>$status,'pr_diketahui'=>$id_user,'nama_diketahui'=>$nama_ketahui);
        $this->db->where('id_permintaan',$id);
    ...
    {    
        ...
        $input = array('status'=>$status,'keterangan_status'=>$keterangan);
        $this->db->where('id_pr_detail',$id);
